...
def extract_baseinf(path):
    ...
    response0 = f"""
    本课程主要讲解{title}，视频时长{total_time}，共包含{num_node}个知识点，
    其中包含初级知识点（难度在1~3之间）{Beginner}个，中级知识点（难度在4~6之间）{Intermediate}个，高级知识点（难度在7~10之间）{Advanced}个。
    """
    return response0

def analysis(path, sample):
    # 打开文件并按行读取内容
    with open(os.path.join(path, 'tree1.json'), 'r', encoding='utf-8') as file:
...
def comparison_for_graph(path, sample):
    # 打开文件并按行读取内容
    with open(os.path.join(path, 'subtitles.srt'), 'r', encoding='utf-8') as file:
        srt = file.readlines()
<<<<<<< HEAD
...
def generate_report(path):
    ...
    response0 = extract_baseinf(path)
    response1 = analysis(path, prompt1)
    response1 = extract_json_from_string(response1)

    response2 = analysis(path, prompt2)
    response2 = extract_json_from_string(response2)

    response3 = analysis(path, prompt3 + "知识点关系列表如下：\n" + str(response2))
    response3 = extract_json_from_string(response3)

    response5 = comparison_for_graph(path, prompt5)
